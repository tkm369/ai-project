...
# Geminiが使えない時のフォールバックコンテンツ（カテゴリ別）
_FALLBACK = {
    "片思い": [
...
def _fallback_content(category: str, tone: str, fmt: str) -> dict:
    """Gemini不使用時のフォールバックコンテンツ"""
...
テキストだけ出力（前置き・説明・かぎかっこ不要）："""
    ...
    try:
        ...
        return _fallback_content(category, tone, fmt)
