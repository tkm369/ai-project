...
def generate_and_score_posts(platform="x", top_posts=None, target_chars=None):
    ...
    prompt = f"""あなたはスピリチュアル・恋愛について日々の気づきをSNSでつぶやいている一般人です。
占い師でも専門家でもなく、自分の経験や感覚を素直にシェアするスタイルです。
今日は{today_str}です。
...
【文体・トーンの指示（最重要）】
- 友達に話しかけるような自然でやわらかい口語体で書く
- 短くテンポよく。1文を長くしすぎない
- 「〜かもしれない」「〜な気がする」など断定しすぎない柔らかい表現を使う
- 読んだ人が「わかる」「私もそう思ってた」と感じるような共感ベースの内容
...
- 特定の宗教・宗派・団体名を出さない
- 「必ず○○になる」「100%」など断定的すぎる保証をしない
- 違法行為・危険な行為を連想させる表現を使わない
- 差別・誹謗中傷につながる表現を使わない

【スパム判定回避（最重要）】
- 「いいね」「フォロー」「保存」「シェア」「コメントで教えて」を求める表現は絶対に使わない
- 冒頭を【】の括弧書きで始めない（例:【緊急】【確信】【警告】【断言】【衝撃】は全てNG）
- 「○○しませんか？」「○○してみませんか？」を繰り返し使わない
- 宣伝・勧誘・販促を連想させる表現を避ける
...
def improve_post(post, platform="x", target_chars=None):
    ...
    prompt = f"""以下のSNS投稿をより自然で読みやすく改善してください。

【改善ポイント】
- 1行目を短くシンプルにして、思わず読み続けたくなる書き出しにする
- 全体的に短くテンポよく。長い文は2文に分ける
- 中間部分に「共感→気づき」の流れを作る
{cta_improve}
